...
DECODE_STATUS decode_mp3(HMP3Decoder mp3_decoder, FILE *fp, decode_data *pData, mp3_instance *pInstance) {
    ...
    /* === MECK PATCH: skip ID3v2 tag on first call ===
     * libhelix-mp3's MP3FindSyncWord scans byte-by-byte for the 11-bit
     * sync pattern 0xFFE. JPEG bytes in ID3v2 APIC (album art) frames
     * are full of false matches (JPEG markers like 0xFFE0/0xFFE1/0xFFD8),
     * each triggering a failed MP3Decode. On files with multi-MB embedded
     * art this scan takes long enough to starve IDLE on the audio core
     * and fire the task watchdog. Skip the tag header explicitly so the
     * decoder starts at the first real MPEG frame.
     *
     * Only fires when pInstance->bytes_in_data_buf == 0, which is true
     * on the first decode call after open. Subsequent calls already have
     * data buffered, so we leave them alone.
     */
    if (pInstance->bytes_in_data_buf == 0 && ftell(fp) == 0) {
        uint8_t hdr[10];
        if (fread(hdr, 1, 10, fp) == 10 &&
            hdr[0] == 'I' && hdr[1] == 'D' && hdr[2] == '3') {
            /* ID3v2 tag size is a synchsafe integer in bytes 6..9: each
             * byte contributes 7 bits, MSB of each byte is always 0. */
            uint32_t tag_size = ((uint32_t)(hdr[6] & 0x7F) << 21) |
                                ((uint32_t)(hdr[7] & 0x7F) << 14) |
                                ((uint32_t)(hdr[8] & 0x7F) <<  7) |
                                ((uint32_t)(hdr[9] & 0x7F));
            /* If the "footer present" flag is set (bit 4 of flags byte 5),
             * the tag has an extra 10-byte footer to skip too. */
            if (hdr[5] & 0x10) {
                tag_size += 10;
            }
            fseek(fp, tag_size, SEEK_CUR);  /* past tag body to first frame */
            ESP_LOGI(TAG, "skipped ID3v2 tag (%lu bytes), starting decode at pos %ld",
                     (unsigned long)tag_size, ftell(fp));
        } else {
            /* Not ID3v2 — rewind so existing logic sees a clean fp. */
            fseek(fp, 0, SEEK_SET);
        }
    }
    /* === END MECK PATCH === */
