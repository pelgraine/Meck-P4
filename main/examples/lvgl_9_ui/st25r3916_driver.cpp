...
extern std::unique_ptr<Lvgl_Ui::System> System_Ui;
...
void Set_Lvgl_Data_Label(std::string data, bool clear_flag)
{
    static std::string lvgl_win_cit_nfc_test_data_label_str = "1. Tap a tag to read its content";

    if (clear_flag == true)
    {
        lvgl_win_cit_nfc_test_data_label_str = data;
    }
    else
    {
        lvgl_win_cit_nfc_test_data_label_str += data;
    ...
    if (System_Ui->get_current_win() == Lvgl_Ui::System::Current_Win::CIT_NFC_TEST)
    {
        _lock_acquire(&lvgl_api_lock);
        // 更新数据的标签
        lv_label_set_text(System_Ui->_registry.win.cit.nfc_test.data_label, lvgl_win_cit_nfc_test_data_label_str.c_str());
...
void St25r3916_Loop(void)
{
    ...
    if (ndefDemoFeature != ndefDemoPrevFeature)
    {
        ...
        Set_Lvgl_Data_Label(std::string(reinterpret_cast<const char *>(ndefDemoFeatureDescription[ndefDemoFeature])), true);
    }
        ...
        if (rfalNfcIsDevActivated(rfal_nfc.rfalNfcGetState()))
        {
            ...
            case RFAL_NFC_LISTEN_TYPE_NFCA:
                ...
                default:
                    ...
                    Set_Lvgl_Data_Label(std::string("ISO14443A/NFC-A card found. UID: "), true);
                    Serial.print(hex2Str(nfcDevice->nfcid, nfcDevice->nfcidLen));
                    Set_Lvgl_Data_Label(std::string(hex2Str(nfcDevice->nfcid, nfcDevice->nfcidLen)), false);
                    Serial.print("\r\n");
                    demoNdef(nfcDevice);
                    rfal_nfc.rfalNfcaPollerSleep();
                    break;
                ...
                Set_Lvgl_Data_Label(std::string("\nOperation completed\nTag can be removed from the field"), false);
...
static void demoNdef(rfalNfcDevice *pNfcDevice)
{
    ...
    if (err != ERR_NONE)
    {
        ...
        Set_Lvgl_Data_Label(std::string("\nNDEF NOT DETECTED (ndefPollerContextInitializationWrapper error code: "), false);
        Serial.print(err);
        Set_Lvgl_Data_Label(std::to_string(err), false);
        Serial.print(")\r\n");
        Set_Lvgl_Data_Label(std::string(")"), false);
    ...
    if (err != ERR_NONE)
    {
        ...
        Set_Lvgl_Data_Label(std::string("\nNDEF NOT DETECTED (ndefPollerNdefDetectWrapper error code: "), false);
        Serial.print(err);
        Set_Lvgl_Data_Label(std::to_string(err), false);
        Serial.print(")\r\n");
        Set_Lvgl_Data_Label(std::string(")"), false);
    ...
    else
    {
        ...
        Set_Lvgl_Data_Label(std::string("\n"), false);
        Set_Lvgl_Data_Label(std::string(reinterpret_cast<const char *>(ndefStates[info.state])), false);
        Serial.print(" NDEF detected.\r\n");
        Set_Lvgl_Data_Label(std::string(" NDEF detected."), false);
        ndefCCDump();
...
ReturnCode ndefRecordDump(const ndefRecord *record, bool verbose)
{
    ...
    Set_Lvgl_Data_Label(std::string("\nRecord #"), false);
    Serial.print(index);
    Set_Lvgl_Data_Label(std::to_string(index), false);
    Serial.print((char *)headerSR);
    Set_Lvgl_Data_Label(std::string(reinterpret_cast<const char *>(headerSR)), false);
    Serial.print("\r\n");
...
ReturnCode ndefRtdTextDump(const ndefType *type)
{
    ...
    Set_Lvgl_Data_Label(std::string((utfEncoding == TEXT_ENCODING_UTF8 ? "UTF8" : "UTF16")), false);
    Serial.print(",");
    Set_Lvgl_Data_Label(std::string(","), false);
...
ReturnCode ndefBufferPrint(const char *prefix, const ndefConstBuffer *bufString, const char *suffix)
{
    ...
    Serial.print(prefix);
    Set_Lvgl_Data_Label(std::string(prefix), false);
    if (bufString->buffer == NULL)
    {
        Serial.print("<No buffer>");
        Set_Lvgl_Data_Label(std::string("<No buffer>"), false);
    }
    else
    {
        for (i = 0; i < bufString->length; i++)
        {
            ...
            Set_Lvgl_Data_Label(std::string(1, bufString->buffer[i]), false);
        }
    }
    Serial.print(suffix);
    Set_Lvgl_Data_Label(std::string(suffix), false);
